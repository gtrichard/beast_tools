# BEAST Markov Jumps Plotting Tool
# 12/10/2023 by Gautier RICHARD
# contact email: gautier.richard  _ at _ anses.fr

# Takes as input the Markov Jumps values directly copied from Tracer (3 columns) and pasted and saved as a tab-delimited text file to make a plot.
# available colors and associated names: https://bookdown.org/hneth/ds4psy/D-3-apx-colors-basics.html

#### SCRIPT PARAMETERS ####

input = "ESIV/phylodynamic/H1avN2E/discrete results/Markov_Jumps_region.txt"
output = "ESIV/phylodynamic/H1avN2E/Markov_Jumps_region.pdf"
plot_width = 4.88
plot_height = 3.61
show_numbers = TRUE
colors = c("white", "darkorange1", "tomato2", "red3")


#### MATRIX FORMATTING AND PLOTTING ####

library("pheatmap")
library("grid")
library("gtable")

mj = read.table(input,sep = "\t",dec = ",")
mj[,1] = data.frame(do.call('rbind', strsplit(as.character(mj[,1]),'[.]',fixed=F)))[,2]
mj = mj[grep("..to",mj[,1]),]
mj[,1] = sub(mj[,1],pattern = "\\[1\\]",replacement = "")
mj[,"From"] = as.factor(data.frame(do.call('rbind', strsplit(as.character(mj[,1]),'to',fixed=F)))[,1])
mj[,"To"] = as.factor(data.frame(do.call('rbind', strsplit(as.character(mj[,1]),'to',fixed=F)))[,2])
mj = mj[,c("From","To","V2")]

n = length(sort(unique(mj[,1])))

(mj2  <- with(mj, {
  M <- matrix(nrow=n, ncol=n, 
              dimnames=list(From=levels(mj[,1]), To=levels(mj[,2])))
  M[cbind(mj[,1], mj[,2])] <- mj[,3]
  M
}))
mj2[is.na(mj2)] = 0
...
# Modified pheatmap:::draw_main 
draw_matrix <- function (matrix, border_color, gaps_rows, gaps_cols, fmat, 
          fontsize_number, number_color) 
{
  n = nrow(matrix)
  m = ncol(matrix)
  ...
  res[["rect2"]] = rectGrob(gp = gpar(fill = NA, col = "black"))  
  if (attr(fmat, "draw")) {
    res[["text"]] = textGrob(x = coord$x, y = coord$y, label = fmat, 
...
assignInNamespace(x="draw_matrix", value=draw_matrix, ns="pheatmap")
...
pheatmap(mj2, cluster_rows = F,cluster_cols = F, fontsize_number = 6.5, display_numbers = show_numbers, border_color = NA, number_color = "white",angle_col = 45,main = "Markov Jumps count", color = colorRampPalette(c("white", "darkorange1", "tomato2", "red3"))(100),)
setHook("grid.newpage", NULL, "replace")
grid.text(expression(bold("To")), y=-0.03, gp=gpar(fontsize=11))
grid.text(expression(bold("From")), x=-0.02, rot=90, gp=gpar(fontsize=11))

##Save the plot
pdf(file = output,   # The directory you want to save the file in
    width = plot_width, # The width of the plot in inches
    height = plot_height) # The height of the plot in inches
